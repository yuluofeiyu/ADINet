...
class SwinTransformerV2(nn.Module):
    ...
    def forward(self, x):
        layers_features = self.forward_features(x)
...
if __name__ == '__main__':
    ...
    swin = SwinTransformerV2().cuda()
    print(sum(p.numel() for p in swin.parameters()) / 1e6)
    swin.load_state_dict(torch.load('./pretrain/swinv2_base_patch4_window16_256.pth')['model'],strict=False)
